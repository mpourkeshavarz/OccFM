...
class FlashAttentionFunction(Function):
    ...
    def backward(ctx, do):
        ...
        # Ensure upstream gradient matches compute dtype (e.g., fp16 under autocast)
        if do.dtype is not q.dtype:
        ...
        for ind, (qc, oc, doc, row_mask, lsec, dqc) in enumerate(row_splits):
            # Match gradient chunk dtype as well
            if doc.dtype is not qc.dtype:
                doc = doc.to(dtype=qc.dtype)
            q_start_index = ind * q_bucket_size - qk_len_diff
